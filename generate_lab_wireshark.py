...
def wireshark_config(config: dict[str, Any]) -> dict[str, Any]:
    """
    Configurazione opzionale nel file YAML:
    ...
    Se manca la sezione wireshark, Wireshark non viene avviato automaticamente.
    """
    cfg = config.get("wireshark", {})
    if cfg is None:
        cfg = {}

    if not isinstance(cfg, dict):
        raise ValueError("Il campo wireshark deve essere un dizionario.")

    return {
        "enabled": bool(cfg.get("enabled", False)),
        "interface": str(cfg.get("interface", "any")),
        "auto_start_capture": bool(cfg.get("auto_start_capture", True)),
        "delay": int(cfg.get("delay", 2)),
    }


def generate_start_script(config: dict[str, Any]) -> str:
    """
    Genera uno script di avvio del laboratorio.

    Lo script:
    1. avvia Kathara;
    2. se richiesto dallo YAML, avvia Wireshark;
    3. se auto_start_capture=true, Wireshark parte già in cattura live.
    """
    wcfg = wireshark_config(config)
    ...
    if wcfg["enabled"]:
        interface = wcfg["interface"]
        delay = wcfg["delay"]

        lines.extend([
            'echo "[2/2] Avvio Wireshark..."',
            f"sleep {delay}",
            "",
            "if ! command -v wireshark >/dev/null 2>&1; then",
            '  echo "[WARN] Wireshark non trovato. Installa con: sudo apt install wireshark"',
            "  exit 0",
            "fi",
            "",
        ])

        if wcfg["auto_start_capture"]:
            lines.extend([
                f'WIRESHARK_INTERFACE="${{WIRESHARK_INTERFACE:-{interface}}}"',
                'echo "[INFO] Interfaccia Wireshark: ${WIRESHARK_INTERFACE}"',
                'wireshark -k -i "${WIRESHARK_INTERFACE}" >/tmp/kathara_wireshark.log 2>&1 &',
                'echo "[OK] Wireshark avviato in background. Log: /tmp/kathara_wireshark.log"',
            ])
        else:
            lines.extend([
                "wireshark >/tmp/kathara_wireshark.log 2>&1 &",
                'echo "[OK] Wireshark avviato in background. Log: /tmp/kathara_wireshark.log"',
            ])
    else:
        lines.extend([
...
def generate_stop_script() -> str:
    """
    Genera uno script di arresto del laboratorio.
    Non forza la chiusura di Wireshark, perché l'utente potrebbe voler salvare la cattura.
    """
    return """#!/bin/bash
...
def generate_lab(config_path: Path, output_dir: Path, clean: bool, force: bool) -> Path:
    ...
    start_script = lab_dir / "start_lab.sh"
    start_script.write_text(generate_start_script(config), encoding="utf-8")
    os.chmod(start_script, 0o755)

    stop_script = lab_dir / "stop_lab.sh"
    stop_script.write_text(generate_stop_script(), encoding="utf-8")
